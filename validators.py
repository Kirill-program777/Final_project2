...
def validate_voice_duration(duration):
    if duration <= 0:
        return False, "Длительность голосового сообщения должна быть положительным числом"
    return True, ""


def validate_text_length(text):
    if len(text) == 0:
        return False, "Текстовое сообщение не должно быть пустым"
    return True, ""
...
def is_stt_block_limit(message, duration):
    ...
    duration_status, error_message = validate_voice_duration(duration)
...
def is_tts_symbol_limit(message, text):
    ...
    text_length_status, error_message = validate_text_length(text)
    if not text_length_status:
        return None, error_message
    all_symbols = count_all_limits(user_id, 'tts_symbols')
    if all_symbols is None:
        return None, 'Произошла ошибка при получении количества использованных символов.'
